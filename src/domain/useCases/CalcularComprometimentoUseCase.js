...
class CalcularComprometimentoUseCase {
  ...
  async execute(testeId, respostas, perguntas, candidatoToken) {
    try {
      ...
      const inconsistencias = this.inconsistencyDetector.detectar(respostas, perguntas, scores);
      ...
      return {
        ...
        recomendacoes: this._gerarRecomendacoes(scoresNormalizados, inconsistencias)
      };
  ...
  _validarRespostas(respostas, perguntas) {
    ...
    if (perguntasRespondidas < totalPerguntas * 0.8) {
      throw new Error(`Teste incompleto: ${perguntasRespondidas}/${totalPerguntas} respondidas. Mínimo: 80%`);
    }
  }
  ...
  _normalizarScores(scores) {
    ...
    return {
      ...
      percentis: {
        ...
        continuativo: normalizador.calcularPercentil(scores.detalhes.continuativo)
      }
    };
  }
  ...
  _gerarRecomendacoes(scores, inconsistencias) {
    ...
    if (scores.detalhes.afetivo < 2.5) {
      recomendacoes.push('Baixo comprometimento afetivo: Candidato pode não se identificar emocionalmente com a organização');
    }
    
    if (scores.detalhes.normativo < 2.5) {
      recomendacoes.push('Baixo comprometimento normativo: Candidato pode não sentir obrigação moral de permanecer');
    }
    
    if (scores.detalhes.continuativo > 4.0 && scores.detalhes.afetivo < 3.0) {
      recomendacoes.push('ALERTA: Comprometimento baseado apenas em custos de saída - risco de turnover se surgir melhor oportunidade');
    }
    
    if (inconsistencias.total > 3) {
      recomendacoes.push('Alto índice de inconsistências: Recomenda-se entrevista adicional para validação');
    }
...
class ScoreCalculator {
  calcular(respostas, perguntas) {
    ...
    return {
      ...
      detalhes: {
        afetivo: contadores.afetivo > 0 ? detalhes.afetivo / contadores.afetivo : 0,
        normativo: contadores.normativo > 0 ? detalhes.normativo / contadores.normativo : 0,
        continuativo: contadores.continuativo > 0 ? detalhes.continuativo / contadores.continuativo : 0
      }
    };
  }
  ...
  _extrairPontos(pergunta, resposta) {
    ...
    if (pergunta.pontuacaoPorOpcao && pergunta.pontuacaoPorOpcao[resposta.toLowerCase()]) {
...
class InconsistencyDetector {
  detectar(respostas, perguntas, scores) {
    ...
    inconsistencias.push(...this._detectarDesviosExtremos(respostas, perguntas));
  ...
  _detectarPadroesMonotonos(respostas) {
    ...
    if (valoresUnicos.size === 1 && valores.length > 5) {
      inconsistencias.push({
        ...
        severidade: 'alta',
        descricao: 'Todas as respostas são idênticas - possível falta de atenção'
      });
    }
    ...
    if (sequencial && valores.length > 8) {
      inconsistencias.push({
        ...
        severidade: 'alta',
        descricao: 'Respostas seguem padrão sequencial - possível resposta aleatória'
      });
    }
  ...
  _detectarContradicoesDimensionais(scores) {
    ...
    if (afetivo > 4.0 && normativo < 2.0) {
      inconsistencias.push({
        ...
        severidade: 'media',
        descricao: 'Alto comprometimento afetivo mas baixo normativo - padrão incomum'
      });
    }
    ...
    if (continuativo > 4.0 && afetivo > 4.0 && normativo < 2.5) {
      inconsistencias.push({
        ...
        severidade: 'baixa',
        descricao: 'Alto comprometimento continuativo e afetivo simultaneamente'
      });
    }
    ...
    if (afetivo < 1.5 && normativo < 1.5 && continuativo < 1.5) {
      inconsistencias.push({
        ...
        severidade: 'alta',
        descricao: 'Todas as dimensões extremamente baixas - possível desmotivação ou resposta aleatória'
      });
    }
  ...
  _detectarDesviosExtremos(respostas, perguntas) {
    ...
    const variancia = pontos.reduce((sum, p) => sum + Math.pow(p - media, 2), 0) / pontos.length;
    ...
    if (desvioPadrao < 0.3 && pontos.length > 8) {
      inconsistencias.push({
        ...
        severidade: 'media',
        descricao: `Respostas com variabilidade muito baixa (DP: ${desvioPadrao.toFixed(2)})`
      });
    }
